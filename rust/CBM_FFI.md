...
- `cbm_rs_mcp_sanitize_ascii_in_place_v1`：給 `CBM_USE_RUST_MCP_CODEC=1` 使用的 MCP `search_code` output ASCII sanitizer ABI。Rust 接受 nullable mutable NUL-terminated C string，會就地把所有大於 127 的 byte 改成 `?`，並保留 ASCII 與 `0x7f`；null 為 no-op。C 端只在 `sanitize_ascii()` 中委派此 pure byte mutator；grep execution、source/context reading、JSON building、snippet UTF-8 lossy sanitizer、response/transport/server lifecycle 與 handlers 仍由 C 負責。
- `cbm_rs_mcp_search_code_score_v1`：給 `CBM_USE_RUST_MCP_CODEC=1` 使用的 MCP `search_code` deduped result ranking scorer ABI。Rust 接受 nullable NUL-terminated label/file path 與 `in_degree`，回傳 C helper 相同的整數 score：Function/Method 加 10、Route 加 15、vendored/vendor/node_modules 扣 50、test/spec/`_test.` path 扣 5。C 端只在 `compute_search_score()` 中委派此 pure scorer；grep execution、graph node lookup、dedup/classification、sort comparator、result JSON、response/transport/server lifecycle 與 handlers 仍由 C 負責。
...
- `cbm_rs_mcp_utf8_is_cont_v1`：給 `CBM_USE_RUST_MCP_CODEC=1` 使用的 MCP UTF-8 continuation-byte classifier ABI。Rust 接受 `int` byte（只取低 8-bit），符合 `10xxxxxx` 回 1，否則回 0。C 端只在 `utf8_is_cont()` 中委派此 pure helper；`sanitize_utf8_lossy()` 主流程仍優先走 `cbm_rs_mcp_sanitize_utf8_lossy_v1`，當 ABI 失敗時 fallback 到原 C loop，且 loop 內 continuation 判斷可使用此 helper；snippet/source 讀取、JSON result building、response/transport/server lifecycle 與 handlers 仍由 C 負責。
...
- `cbm_rs_mcp_sanitize_utf8_lossy_v1`：給 `CBM_USE_RUST_MCP_CODEC=1` 使用的 MCP code snippets 來源 UTF-8 lossy 消毒 ABI。Rust 接受 nullable NUL-terminated C string 與 caller-owned output buffer。對非 UTF-8 byte 進行 lossy 消毒，將其替換為 REPLACEMENT CHARACTER U+FFFD。成功時回傳消毒後所需的位元組長度，短 buffer 會 NUL-terminate 截斷；`input == NULL` 回傳 `SIZE_MAX`。C 端只在 `sanitize_utf8_lossy()` 中委派此 pure validator/serializer；yyjson building/serialization、response/transport/server lifecycle 與 handlers 仍由 C 負責。
...
- `cbm_rs_mcp_project_db_file_name_v1`：給 `CBM_USE_RUST_MCP_CODEC=1` 使用的 MCP cache project DB filename classifier ABI。Rust 接受 nullable NUL-terminated C string，回傳 1 表示 filename 長度至少為 `x.db`、精準以 `.db` 結尾，且不是 `_` 開頭或 `:memory:` 開頭；null、`.db`、`*.db-wal`、`*.sqlite`、internal/hidden filename 與 SQLite memory marker 回 0。`tmp-*.db` 維持有效，因 `/tmp/...` source root 會導出合法 tmp-prefixed project name。C 端只在 `is_project_db_file()` 中委派此 pure classifier；directory scanning、SQLite query-open、internal project-name resolution、ghost/corrupt DB filtering、JSON list building、resolve fallback、response/transport/server lifecycle 與 handlers 仍由 C 負責。
...
## Pipeline pkgmap text scan slice

`src/pipeline/pass_pkgmap.c` 的 bounded `at_prefix` 與 `find_line_value` 已新增 `CBM_USE_RUST_PIPELINE_PKGMAP_TEXT=1` opt-in。Rust ABI 對 `at_prefix` 回傳布林值，對 line lookup 回傳 source offset，C 端再將 offset 轉回借用 pointer；manifest parsing、heap string helpers、檔案 I/O 與 package entry ownership 仍留在 C。已建立 `rust-pipeline-pkgmap-text-optin-test`，尚未執行。

此 pkgmap slice 另包含 `path_dirname` 與 `strip_extension`；Rust ABI 以 length-query + caller buffer 方式回傳，C wrapper 配置與釋放 heap 字串，避免跨邊界 ownership 不清。
...
`src/pipeline/pass_enrichment.c` 的 `cbm_split_camel_case` 與 `cbm_tokenize_decorator` 已新增 `CBM_USE_RUST_PIPELINE_ENRICHMENT_TOKENS=1` opt-in。Rust 產生的 token 以 C `malloc` 配置並由既有 caller free；graph label collection、hash table word frequency、yyjson decorator_tags injection 與 store pass 仍在 C。`rust-pipeline-enrichment-tokens-optin-test` 已建立但尚未執行。


## Calls import JSON helper slice

`src/pipeline/pass_calls.c` 的 `extract_local_name_from_json` 已新增 `CBM_USE_RUST_PIPELINE_CALLS_JSON=1` opt-in，Rust 以 length-query ABI 回傳 C-owned heap string 長度，C wrapper 負責配置/釋放。import map、FQN/registry resolution、graph lookup 與 CALLS edge emission 仍在 C；`rust-pipeline-calls-json-optin-test` 已建立但尚未執行。
