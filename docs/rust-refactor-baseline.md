...
> **基準文件，不是目前交接狀態（2026-07-16）**：目前切片、FFI 清單與 gate 結果請讀
> [rust-refactor-current-handoff.md](rust-refactor-current-handoff.md)、根目錄的
> [Tasks.md](../Tasks.md) 與 [rust/CBM_FFI.md](../rust/CBM_FFI.md)。本文件的命令清單是基準方向，不是完整或最新 ABI
> 覆蓋清單；實際內容以 Makefile.cbm 與 tests/test_rust_ffi.c 為準。
...
- `tests/golden/rust-refactor/language-graph-parity.json`：Phase 4 代表性語言 graph parity，固定 Python、TypeScript、Go、Rust、Java、C++ 與 YAML fixture 的 definitions、calls、imports、routes、semantic/LSP edges，並比對 default C path 與 Rust pipeline/store opt-in path。
...
- `make -f Makefile.cbm rust-ffi-test`：編譯 Rust `staticlib`，由 C smoke test 呼叫目前的 Rust ABI export；目前覆蓋 `dump_verify`、`str_intern`、`str_util`、`platform`、diagnostics formatter、compat deterministic helpers、arena public struct、Store FTS tokenization、pipeline project-name sanitizer、pipeline pass plan、FullPipeline top-level metadata、typed sequential/incremental/predump/parallel extraction v2 metadata、typed incremental post steps、graph-buffer mutation command metadata/validation、registry one-shot resolver 與 registry handle 的 null、embedded NUL、指標穩定性、oversized length、validation、JSON escape、原地路徑正規化、env/dirs buffer 合約、cgroup parser 防護、diagnostics exact JSON/NDJSON/path bytes、regex cap/status、thread stack、aligned alloc precheck、arena alignment/growth/reset/destroy、Store camel/acronym/fallback/truncation、project-name byte sanitizer/truncation、pipeline sequential/predump/incremental/parallel plan order、FullPipeline top_v1 order/requires/nested plan kind、sequential/incremental/predump/parallel v2 kind/phase/policy/gate/requires/effect 欄位、graph-buffer command kind/field/effect/null/empty/reserved-bit contract、incremental tail kind/policy、parallel threshold、handle lifecycle、output truncation，以及 import map、qualified suffix、candidate cap 與 import reachability penalty。
- `make -f Makefile.cbm rust-foundation-optin-test`：重建並執行 foundation 預設 C path、各單一 Rust opt-in，以及全部 opt-in 的組合。
- `make -f Makefile.cbm rust-yaml-optin-test`：重建 foundation 預設 C path 與 `CBM_USE_RUST_YAML=1` baseline，執行 `test-foundation` 與 `cbm --version` 驗證；確認 parser/query ABI 在 foundation 與 CLI 常見路徑可平行。
- `make -f Makefile.cbm rust-yaml-only-test`：重建 `CBM_USE_RUST_YAML_ONLY=1` runner，執行預設 C 與 YAML-only 的 `test-foundation`、`test-rust-ffi` 與 `cbm --version`，用於獨立驗證 `cbm_yaml_*` 與 `CBMRustYamlNode` 行為契約。
...
- `make -f Makefile.cbm rust-store-fts-tokenizer-optin-test`：以 `CBM_USE_RUST_STORE_FTS_TOKENIZER=1` 重建 full C test runner，執行 `store_compat mcp`，驗證 `cbm_camel_split` SQLite UDF 的 Rust token 字串計算 opt-in 不改變 FTS rebuild、BM25 search_graph 與 Store/MCP contract。
- `make -f Makefile.cbm rust-language-graph-parity`：重建 default C binary 與 `CBM_USE_RUST_PIPELINE_REGISTRY=1 CBM_USE_RUST_PIPELINE_PLAN=1 CBM_USE_RUST_STORE_FTS_TOKENIZER=1` binary，索引 hermetic 代表性語言 fixture，並比對 `language-graph-parity.json`。需要刻意接受變更時使用 `make -f Makefile.cbm rust-language-graph-parity-update`。
...
- `cbm_rs_yaml_parse`、`cbm_rs_yaml_free`、`cbm_rs_yaml_get_str`、`cbm_rs_yaml_get_float`、`cbm_rs_yaml_get_bool`、`cbm_rs_yaml_get_str_list`、`cbm_rs_yaml_has`：對應 `src/foundation/yaml.c` 的 parser/query ABI。Rust 只負責語意解析與節點查詢，`CBM_USE_RUST_YAML`/`CBM_USE_RUST_YAML_ONLY` 驅動時，C wrapper 保留 `cbm_yaml_node_t` 實作所有權與既有呼叫介面契約。
...
- `cbm_rs_compat_*`：對應 `src/foundation/compat_regex.c` / `compat_thread.c` 的 deterministic helper contract。Rust parity 只覆蓋 regex known flags、match cap/status、thread default stack 與 aligned alloc precheck；regex engine、filesystem/process wrapper、thread lifecycle 與 Windows shim 仍保留 C。
- `cbm_rs_arena_*`：對應 `src/foundation/arena.c` 的 public `CBMArena` layout contract。Rust parity 只作 test-only FFI fixture，固定 default/min block size、8-byte alignment、block growth、calloc、`strdup`/`strndup`、reset、destroy 與 total accounting；Rust 配置的 blocks 必須由 Rust `cbm_rs_arena_reset/destroy` 釋放，不得與 C `cbm_arena_destroy` 混用。
- `cbm_rs_store_camel_split_v1`：對應 Store `cbm_camel_split` SQLite UDF 的 FTS tokenization contract。預設 C path 仍使用 C `cbm_camel_split`；`CBM_USE_RUST_STORE_FTS_TOKENIZER=1` 只將 C UDF callback 內的 token 字串計算委派給 Rust helper，固定原 identifier + split identifier、ASCII lower→upper 與 acronym boundary、NULL/empty、非 ASCII byte 行為、2048-byte fallback/truncation；SQLite open/pragma/schema、UDF registration、FTS5 runtime、`sqlite3_result_text`、CRUD/search/BM25 仍由 C 負責。
- `cbm_rs_pipeline_project_name_from_path`：對應 `src/pipeline/fqn.c` 的 `cbm_project_name_from_path` byte sanitizer。Rust parity 寫入 caller-provided buffer，回傳完整輸出長度，固定 NULL/empty/root fallback、separator/drive normalization、unsafe ASCII collapse、non-ASCII byte hex encoding 與 truncation；`CBM_USE_RUST_PIPELINE_PROJECT_NAME=1` 已將這條 helper 升級為 production opt-in。
- `cbm_rs_pipeline_plan_describe` / `cbm_rs_pipeline_plan_steps_v2` / `cbm_rs_pipeline_full_plan_steps_v1`：對應 `src/pipeline/pipeline.c` / `pipeline_incremental.c` 的 pass plan 資料切片。Rust parity 固定 full pipeline top-level order、sequential dispatch 與 infra tail、parallel extraction order、predump fast-mode gating、typed sequential/incremental/predump/parallel extraction v2 metadata、parallel threshold `file_count > 50`、incremental extract/resolve dispatch plan，以及 incremental post/tail typed order；`cbm_rs_pipeline_full_plan_steps_v1` 必須以實際 `mode/worker_count/file_count` 呼叫，`pipeline.c` opt-in runtime 只讀取 top_v1 metadata 與 extraction choice，`pipeline_incremental.c` 不直接消費 FullPipeline metadata。不觸碰 tree-sitter shim、`CBMFileResult`、LSP 或 graph-buffer storage/ownership。
...
- `CBM_USE_RUST_YAML=1`：讓 `cbm_yaml_parse`、`cbm_yaml_free`、`cbm_yaml_get_str`、`cbm_yaml_get_float`、`cbm_yaml_get_bool`、`cbm_yaml_get_str_list` 與 `cbm_yaml_has` 轉呼叫 Rust ABI。`CBM_USE_RUST_YAML_ONLY=1` 僅用於測試目標，驗證 Rust-only 編譯 path 上的 parser/query 契約。
...
- `CBM_USE_RUST_PIPELINE_PLAN=1`：讓 `pipeline.c` 在 discovery 後載入 FullPipeline top_v1 metadata，記錄 `rust_plan.dispatch phase=full_pipeline source=top_v1`，並以 `extraction_dispatch.nested_plan_kind` 驅動 full extraction choice；讓 full 與 incremental extraction phase 透過 Rust `pipeline::plan` 回答 parallel/sequential 選擇，讓 sequential extraction dispatch 依 Rust typed v2 metadata 執行 `definitions`、`k8s`、`lsp_cross`、`calls`、`usages` 與 `semantic`，讓 incremental extract/resolve 依 Rust typed v2 metadata 驗證後 dispatch sequential `definitions`、`calls`、`usages`、`semantic` 或 parallel `incr_extract`、`incr_registry`、`incr_resolve`，讓 parallel extraction 依 Rust typed v2 metadata 驗證並記錄 `parallel_extract`、`registry_build`、`lsp_cross_prepare`、`parallel_resolve`、`infra_routes`、`infra_bindings` 與 `k8s` 外層順序，讓 full pre-dump 階段依 Rust typed v2 metadata dispatch `decorator_tags`、`configlink`、`route_match`、`similarity`、`semantic_edges` 與 `complexity`，並透過 typed incremental post steps dispatch `k8s`、`incr_tests`、`incr_decorator_tags`、`incr_configlink`、`incr_similarity`、`incr_semantic_edges`、`edge_relink`、`incremental_dump`、`persist_hashes` 與 `artifact_export`；C 端會逐欄驗證 FullPipeline top_v1 order/requires/nested plan、sequential/incremental/predump/parallel extraction `kind/phase/policy/gate_flags/requires_mask/effect_flags`，實際 C pass function pointers、parallel cache/cross-LSP cleanup、dump、hash persist、FTS rebuild、artifact export、tree-sitter/extraction/LSP、infra processing 與 graph-buffer 寫入不遷移。Incremental dump failure 由 C tail handler 傳回實際 rc，並跳過後續 persist/artifact tail，避免以 full fallback 隱藏錯誤。
- `CBM_USE_RUST_STORE_FTS_TOKENIZER=1`：讓 Store `cbm_camel_split` SQLite UDF callback 的 token 字串計算轉呼叫 Rust ABI。SQLite connection、UDF callback registration、`sqlite3_context`、FTS5 table/runtime、CRUD/search/BM25 與所有 Store ownership 仍留在 C。
- 預設未設定時仍使用原 C implementation；production `cbm` 與 `cbm-with-ui` 只有在任一 Rust opt-in 啟用時才連 Rust staticlib。

Pipeline plan strategy：

- `pipeline::plan` 目前固定 C orchestration 的 pass order 與 gating。產品 opt-in 讓 C 端只讀 Rust plan 來消費 FullPipeline top_v1 metadata、選 full/incremental extraction path、sequential extraction typed v2 metadata、incremental extract/resolve typed v2 dispatch、parallel extraction typed v2 metadata、full pre-dump typed v2 metadata 與 typed incremental post/tail order，仍呼叫既有 C pass function pointers 與 dump/persist/artifact handlers；`full_pipeline source=top_v1`、sequential/incremental/predump/parallel extraction `source=typed_v2` trace 與 incremental dump failure tests 固定 C adapter 的觀測行為與錯誤傳遞。不得在同一切片遷移 extraction、tree-sitter shim、LSP cross-file prebuild、dump/persist/artifact 實作或 graph-buffer storage/ownership；graph-buffer mutation command boundary 目前只可作 metadata/validation，C-side adapter 只可作同步 normalization/dispatch，目前代表性呼叫點包含 structure、incremental purge/file-upsert/inbound restore、`TESTS`/`TESTS_FILE`、usage 與 configlink edges。
- tree-sitter 相關驗證由 language graph parity gate 起步，需保留 default C path、`CBM_USE_RUST_PIPELINE_REGISTRY=1 CBM_USE_RUST_PIPELINE_PLAN=1 CBM_USE_RUST_STORE_FTS_TOKENIZER=1` opt-in path，以及後續新增的 `CBM_DISABLE_LSP_CROSS=1`/預設 cross-LSP 路徑，確認 plan/store tokenization 切片沒有改變 definitions、calls、imports、routes、semantic/LSP edges。
...
- `CBM_USE_RUST_PIPELINE_TEST_NODE_IS_TEST=1`：將 `pass_tests.c` 的 `is_test` properties JSON 純 predicate 改由 Rust ABI 處理；C 仍保留節點與 graph 所有權。
- `CBM_USE_RUST_UI_LAYOUT_HELPERS=1`：將 `layout3d.c` 的 stellar degree 顏色與 label size 純映射改由 Rust ABI 處理；Barnes-Hut、SQLite 與 layout 結果所有權仍由 C 負責。
- `CBM_USE_RUST_UI_LAYOUT_HELPERS=1` 亦涵蓋 layout 的 FNV-1a cluster seed 與 deterministic jitter helper。
- `CBM_USE_RUST_UI_LAYOUT_HELPERS=1` 亦涵蓋 octree 的 octant 與 child-center scalar/vector 計算；octree node ownership 仍在 C。
- `CBM_USE_RUST_UI_LAYOUT_HELPERS=1` 亦涵蓋 `node_id_entry_t` 的 C-layout 二分搜尋；map allocation 與排序仍由 C 管理。
- `CBM_USE_RUST_UI_LAYOUT_HELPERS=1` 亦涵蓋 requested/cap 的 max-node clamp；環境變數解析仍由 C 負責。
- `CBM_USE_RUST_UI_HTTPD_HELPERS=1`：將 HTTP parser 的 header-name 比對與 bounded header-value copy 改由 Rust ABI 處理；socket、request ownership 與 handler 仍由 C 負責。
- `CBM_USE_RUST_AST_PROFILE_CLASSIFIERS=1`：將 AST profile node-kind 純分類改由 Rust bitmask ABI 處理；Tree-sitter traversal、Halstead set 與 profile ownership 仍由 C 負責。
- `CBM_USE_RUST_AST_PROFILE_CLASSIFIERS=1` 亦涵蓋固定 512-slot Halstead insert；set ownership 仍在 C。
- `CBM_USE_RUST_AST_PROFILE_CLASSIFIERS=1` 亦涵蓋 parameter-name membership；C 仍保留 source/array lifetime。
- `CBM_USE_RUST_PIPELINE_USAGES_JSON=1`：將 IMPORTS edge properties 的 `local_name` 純擷取改由 Rust ABI 處理；C 仍擁有 import map 與 graph edges。
- `CBM_USE_RUST_SEMANTIC_TOKEN_CLASSIFIERS=1`：將 semantic token delimiter/camel break predicate 改由 Rust ABI 處理；token buffer 與 abbreviation ownership 仍由 C 負責。
