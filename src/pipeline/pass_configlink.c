...
/* manifest/dep helpers：見 configlink_helpers.c（true-source selectable CU） */
...
static int collect_manifest_deps(const cbm_gbuf_node_t *const *vars, int var_count,
    ...
    for (int i = 0; i < var_count && n < max_out; i++) {
        ...
        bool is_dep =
            vars[i]->qualified_name && cbm_pipeline_configlink_is_dep_section(vars[i]->qualified_name);

        if (!is_dep && strcmp(base, "Cargo.toml") == 0 && vars[i]->qualified_name) {
            is_dep = cbm_pipeline_configlink_is_cargo_dep_section(vars[i]->qualified_name) != 0;
        }
...
static double match_dep_to_import(const cbm_gbuf_node_t *target, const char *dep_lower) {
    ...
    return cbm_pipeline_configlink_match_dep_to_import(target->name, target->qualified_name,
                                                       dep_lower);
}
...
static int strategy_dep_imports(cbm_gbuf_t *gb) {
    ...
    for (int di = 0; di < dep_count; di++) {
        ...
        cbm_pipeline_configlink_lowercase_into(dep_lower, sizeof(dep_lower), deps[di].name);
