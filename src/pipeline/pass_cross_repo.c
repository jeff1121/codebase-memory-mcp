...
static const char *json_str_prop(const char *json, const char *key, char *buf, size_t bufsz) {
    return cbm_pipeline_cross_repo_json_str_prop(json, key, buf, bufsz);
}
...
static void build_cross_props(char *buf, size_t bufsz, const char *target_project,
    ...
    cbm_pipeline_cross_repo_build_props(buf, bufsz, target_project, target_function, target_file,
                                        url_or_channel, extra_key, extra_val);
}
